...
#define UA_SECURITYPOLICY_BASIC128RSA15_MINASYMKEYBITLENGTH          1024
#define UA_SECURITYPOLICY_BASIC128RSA15_MAXASYMKEYBITLENGTH          4096
#define UA_SHA1_LENGTH                                               20
...
UA_ChannelModule_Basic128Rsa15_New_Context (const UA_SecurityPolicy * securityPolicy,
    ...
    EVP_PKEY_free(key);
    /* check the key size */
    if(keySize < UA_SECURITYPOLICY_BASIC128RSA15_MINASYMKEYBITLENGTH ||
       keySize > UA_SECURITYPOLICY_BASIC128RSA15_MAXASYMKEYBITLENGTH) {
        UA_ByteString_clear(&context->remoteCertificate);
        UA_free(context);
